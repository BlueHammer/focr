...
function MirrorImage( event )
	...
	local particle = ParticleManager:CreateParticle("particles/naruto/glimmer_cape_initial_flash_smoke.vpcf", PATTACH_POINT_FOLLOW, caster)
	...
	local vRandomSpawnPos = {
		Vector( 144, 0, 0 ),		-- North
		Vector( 0, 144, 0 ),		-- East
		Vector( -144, 0, 0 ),	-- South
		Vector( 0, -144, 0 ),	-- West
		Vector( 102, 102, 0 ),
		Vector( 102, -102, 0 ),
		Vector( -102, -102, 0 ),
	...
	for i=1, images_count do
		...
		local particle = ParticleManager:CreateParticle("particles/naruto/glimmer_cape_initial_flash_smoke.vpcf", PATTACH_POINT_FOLLOW, illusion)
		ParticleManager:SetParticleControlEnt(particle, 0, illusion, PATTACH_POINT_FOLLOW, "attach_origin", illusion:GetAbsOrigin(), true)
		...
		for abilitySlot=0,15 do
			...
			if ability ~= nil then 
				...
				if abilityName == "naruto_odama_rasengan" then
					illusionAbility = illusion:FindAbilityByName("rubick_empty1")
					abilityLevel = 0
				end
